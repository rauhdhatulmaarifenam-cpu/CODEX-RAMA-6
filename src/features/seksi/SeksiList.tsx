...
export function SeksiList(){
  ...
  const handleExport=(type:'xlsx'|'md'|'pdf')=>{ if(!data?.data) return; const cols=[{key:'nama_seksi',header:'Nama Seksi'},{key:'deskripsi',header:'Deskripsi'},{key:'pembina',header:'Pembina'}]; const mapped=data.data.map((r:any)=>({...r, pembina:r.pembina?.nama_lengkap||r.pembina?.nickname||'-', deskripsi:r.deskripsi||'-'})); if(type==='xlsx') exportToXlsx('seksi','semua',mapped,cols); else if(type==='md') exportToMarkdown('seksi','semua',mapped,cols); else exportToPdf('seksi','semua',mapped,cols); };
  return (
